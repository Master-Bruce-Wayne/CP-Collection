...
void rec(string temp, map<ll,ll> mpp,vstr& ans) {
    if(mpp.size()==0)  {  ans.PB(temp); return;  }

    bool ok=true;
    for(auto it:mpp) {
        if(it.ss==0)  continue;
        temp.PB((char)('a'+it.ff)) ;
        mpp[it.ff]--;
        // if(mpp[it.ff]==0)  mpp.erase(it);
        rec(temp,mpp,ans) ;
        mpp[it.ff]++;
        temp.pop_back();
        ok=false;
