...
void genFact() {
    fact[0]=fact[1]=1;
    for(ll i=1; i<1000005; i++)  fact[i]=(1LL*fact[i-1]*i)%MOD;
}
...
void solve() {
    ...
    // ll ans=1;
    // for(ll i=1; i<=b; i++) {
    //     ans=((1LL*ans*(a-b+i))%MOD *modInverse(i,MOD))%MOD;
    // }

    ll ans = (((fact[a]*modInverse(fact[b],MOD))%MOD)*modInverse(fact[a-b],MOD))%MOD;
    cout<<ans;
}
