...
/*
        e u l e r   t o u r  +  s e g m e n t   t r e e
*/
...
ll timer=1;
void euler_dfs(ll node, ll par) {
    intime[node]=timer++;
    euler.pb(val[node]);

    for(auto x:graph[node]) {
        if(x!=par)  euler_dfs(x,node);
    }
...
void solve() {
    ...
    for(ll i=0; i<q; i++) {
        ...
        if(ch==2) {
            ll x; cin>>x;
            // debugThree(x,intime[x], outtime[x]-1);
            ll ans=query(1,1,n,intime[x], outtime[x]-1);
            cout<<ans<<endl;
        }
        else{
            ll x,y; cin>>x>>y;
            update(1,1,n,intime[x],y);
            // update(1,1,2*n,outtime[x],-y);
        }
    }
