...
struct Node {
    ll lms,rms,ms,tsum;
    Node() { lms=0,rms=0,ms=0,tsum=0;  }
    Node(ll val) { lms=rms=ms=max(0LL,val); tsum=val; }
    Node(ll v1,ll v2,ll v3,ll v4) { lms=v1,rms=v2,ms=v3; tsum=v4; }
};

Node merge(Node a, Node b) {
    ll lsum=max(a.lms,a.tsum+b.lms);
    ll rsum=max(b.rms,b.tsum+a.rms);
    ll msum=max({a.ms,b.ms,a.rms+b.lms});
    ll totsum=a.tsum+b.tsum;
    Node res=Node(lsum,rsum,msum,totsum);
    return res;
}
