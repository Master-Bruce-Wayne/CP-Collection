...
vl z_function(string s) {
    ...
    ll left=0, right=0;
    for(ll k=1; k<n; k++) {
        // naya window create karne ke liye
        if(k>right) {
            left=right=k;
        ...
        // window ke andar
            ...
            ll k1=k-left;
            // window ke right bound ko touch X
            if(zArr[k1]<right-k+1) {  zArr[k]=zArr[k1];  }
            else  {  // start new window from k
                left=k;
