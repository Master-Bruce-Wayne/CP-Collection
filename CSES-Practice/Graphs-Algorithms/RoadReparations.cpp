...
void traversal() {
    MinHeap<pl> pq; pq.push({0,1});
    while(!pq.empty()) {
        auto it=pq.top(); pq.pop();
        if(vis[it.ss])  continue;
        vis[it.ss]=1; sum+=it.ff;
        for(auto pn:graph[it.ss]) {
            ll nn=pn.ff, wt=pn.ss;
            pq.push({wt,nn});
        }
    }
}
...
void solve() {
    cin>>n>>m; sum=0;
    graph.clear(); graph.resize(n+1); vis.assign(n+1,0);
    for(ll i=0; i<m; i++) {
        ll a,b,c; cin>>a>>b>>c;
    ...
    bool ok=true;
    for(ll i=1; i<=n; i++)  if(!vis[i]) ok=false;

    if(!ok) { cout<<"IMPOSSIBLE"; return; }
    cout<<sum;
}
