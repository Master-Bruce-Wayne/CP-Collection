...
bool dfs(ll nn) {
    vis[nn]=1; pathvis[nn]=1; st.push(nn);
    for(auto it:graph[nn]) {
        if(!vis[it]) {  if(dfs(it)) return true;  }
        else {
            if(!pathvis[it])  continue;
            // debug(it); 
    ...
    pathvis[nn]=0; st.pop(); return false;
}

void solve() {
    cin>>n>>m;
    graph.clear(); graph.resize(n+1); vis.assign(n+1,0); pathvis.assign(n+1,0);
    for(ll i=0; i<m; i++) {
        ll a,b; cin>>a>>b;
        graph[a].PB(b);
    }
