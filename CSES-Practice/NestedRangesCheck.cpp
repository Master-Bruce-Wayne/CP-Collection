...
bool comp1(pair<pl,ll>& a, pair<pl,ll>& b) {
    if(a.ff.ff==b.ff.ff)  return a.ff.ss<b.ff.ss;
    return a.ff.ff>b.ff.ff;
}

bool comp2(pair<pl,ll>& a, pair<pl,ll>& b) {
    if(a.ff.ff==b.ff.ff)  return a.ff.ss>b.ff.ss;
    return a.ff.ff<b.ff.ff;
}
...
void solve() {
    ...
    for(ll i=0; i<n; i++)  { cin>>a[i].ff.ff>>a[i].ff.ss; a[i].ss=i; }
    sort(all(a),comp1);
    ...
    for(ll i=0; i<n; i++) {
        ll ar=a[i].ff.ss;
        if(!st.empty() && *st.begin()<=ar) { ans1[a[i].ss]=1;  }
        st.insert(ar);
    }
    ...
    for(ll i=0; i<n; i++) {
        ll ar=a[i].ff.ss;
        if(!st.empty() && *st.rbegin()>=ar) { ans2[a[i].ss]=1; }
        st.insert(ar);
    }
