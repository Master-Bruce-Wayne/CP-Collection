...
ll rec(ll ele, ll target,vvl& dp) {
    if(target==0)  return 1;
    if(ele==1)  return (target==ele);
    if(dp[ele][target]!=-1)  return dp[ele][target];

    ll pick=0, notpick=0;
    if(ele<=target)  pick=rec(ele-1,target-ele,dp);
    notpick=rec(ele-1,target,dp);

    return dp[ele][target] = (pick+notpick)%MOD;
}
...
void solve() {
    ...
    vvl dp(n+1, vl(tsum/2 +1,-1));
    ll ans=rec(n,tsum/2, dp);
