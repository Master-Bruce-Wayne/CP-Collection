...
void solve() {
    ...
    while(mpp[chrc]--)  ans.PB((char)('A'+chrc));
    
    // string tmp="";
    for(auto it:mpp) {
        ll f=it.ss/2, ch=it.ff;
        for(ll i=0; i<f; i++) { ans.PB((char)('A'+ch)); ans=((char)('A'+ch)) + ans; }
    }
